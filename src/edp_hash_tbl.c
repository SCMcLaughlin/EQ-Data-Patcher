...
static int tbl_set_impl(HashTbl* tbl, int64_t key, uint32_t len, uint32_t isIntKey, const void* value, uint32_t hash)
{
    uint32_t freeIndex;
...
static void* tbl_get_impl(HashTbl* tbl, int64_t key, uint32_t len, uint32_t isIntKey, uint32_t hash)
{
...
static int tbl_remove_impl(HashTbl* tbl, int64_t key, uint32_t len, uint32_t isIntKey, uint32_t hash)
{
    uint32_t capMinusOne    = tbl->capacity - 1;
    uint32_t entSize        = tbl->entSize;
    uint32_t freeIndex      = tbl->freeIndex;
